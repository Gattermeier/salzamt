...
- No backend, no analytics, no cookies, no third-party requests of any kind. The three web fonts
  (Manufacturing Consent for the Salzamt wordmark and all headings, UnifrakturCook for display
  accents, EB Garamond, Special Elite; OFL / Apache licensed) are self-hosted as woff2 files
...
--font-wordmark: "Manufacturing Consent", "UnifrakturCook", serif;   /* headings, card/dialog titles, .wordmark */
...
Headings (`h1`–`h3`), card titles and dialog titles in `--font-wordmark` (Manufacturing Consent,
weight 400, its only weight), `--ink`; `h1` = `clamp(2.6rem, 7vw, 4.5rem)`. `--font-display`
(UnifrakturCook) remains for display accents: hero subtitle, Amtsweg step names, footer motto,
the `.display` utility.
...
| `badge-320.png`   | `amtssiegel.jpg`           | 320×320             | Amtssiegel: eagle centred in a circle, text remnants painted over, transparent corners |
| `badge-96.png`    | `amtssiegel.jpg`           | 96×96               | header, favicon                                                                        |
